...
.constant('paginationConfig', {
  ...
  boundaryLinks: false,
  directionLinks: true,
  firstClass: 'glyphicon glyphicon-fast-backward',
  previousClass: 'glyphicon glyphicon-step-backward',
  nextClass: 'glyphicon glyphicon-step-forward',
  lastClass: 'glyphicon glyphicon-fast-forward',
  rotate: true
})
...
.directive('pagination', ['$parse', 'paginationConfig', function($parse, config) {
  return {
    ...
    link: function(scope, element, attrs, paginationCtrl) {
      ...
      previousClass  = paginationCtrl.getAttributeValue(attrs.previousClass,  config.previousClass,  true),
      nextClass      = paginationCtrl.getAttributeValue(attrs.nextClass,      config.nextClass,      true),
      lastClass      = paginationCtrl.getAttributeValue(attrs.lastClass,      config.lastClass,      true),
      ...
      paginationCtrl.getPages = function(currentPage, totalPages) {
        ...
        if (directionLinks) {
          var previousPage = makePage(currentPage - 1, previousClass, '', false, paginationCtrl.noPrevious());
          pages.unshift(previousPage);

          var nextPage = makePage(currentPage + 1, nextClass, '', false, paginationCtrl.noNext());
          pages.push(nextPage);
        }

        // Add first & last links
        if (boundaryLinks) {
          var firstPage = makePage(1, firstClass, '', false, paginationCtrl.noPrevious());
          pages.unshift(firstPage);

          var lastPage = makePage(totalPages, lastClass, '', false, paginationCtrl.noNext());
          pages.push(lastPage);
        }
